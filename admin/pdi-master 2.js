...
(() => {
  // Uses the same authenticated Admin Worker as your dashboard (admin-worker.js), which serves under /api/*
  const API_BASE = "/api";

  // Airtable field names (car table)
  const NOTES_FIELD_CANDIDATES = ["Notes", "Internal Notes", "Admin Notes"];
  const PDI_JSON_FIELD_CANDIDATES = ["PDI_JSON", "PDI JSON", "PDI Json"];

  // New (confirmed) Airtable field additions
  const VIN_FIELD = "VIN";
  const HPI_FIELD = "HPI_report_number";
  const V5C_FIELD = "V5C_number";
  ...
  function labelForGroup(name) {
    // Attempts to infer the item label from the table row text
  ...
  function validate({ strict = false } = {}) {
    ...
    // HPI must not be FAIL when complete
    ...
    // Any rectified items require a note
    const groups = gatherChecklistRadioGroups();
    for (const [name] of groups.entries()) {
      // ignore header radios in section A (veh_fuel, veh_hpi)
    ...
    // Customer acknowledgement is required only on complete
  ...
  function buildPdiObject({ status = "draft" } = {}) {
    ...
    for (const el of els) {
      ...
      if (el.type === "radio") continue; // handled separately
  ...
  function autoAppendRectificationsToFinalNotes() {
    ...
    if (existing.includes(header)) return; // avoid duplicating
  ...
  async function load() {
    try {
      ...
      // Expected shape: { id, fields: { ... } }
      const fields = data.fields || {};

      // Prefill vehicle details (only if blank)
      // Primary field names reflect your current Airtable schema / admin dashboard keys.
      const makeModel = getField(fields, ["Make_Model", "Make / Model", "Vehicle", "Title"]);
      const reg = getField(fields, ["Registration", "Reg", "VRM"]);

      // New requested Airtable fields
      const vin = getField(fields, [VIN_FIELD, "VIN / chassis", "VIN / Chassis"]);
      const hpiReportNo = getField(fields, [HPI_FIELD, "HPI report number", "HPI Report Number"]);
      const v5cNo = getField(fields, [V5C_FIELD, "V5C number", "V5C Number"]);

      const mileage = getField(fields, ["Mileage", "Mileage_at_PDI", "Odometer"]);
      const mot = getField(fields, ["MOT_Date", "MOT expiry", "MOT Expiry", "MOT"]);
      const keys = getField(fields, ["Keys", "Keys supplied", "Key Count"]);
      ...
      // Live validation for rectified items
      document.addEventListener("change", (ev) => {
        const t = ev.target;
        if (!t) return;
        if (t.type === "radio") {
          // If switched to rect, jump focus to note field
          if (t.value === "rect") {
  ...
  async function downloadPdf() {
    showErrors([]);
    const doc = document.getElementById("pdiDocument");
    if (!doc) return;

    // Ensure any rectifications are reflected in final notes for the PDF output
    autoAppendRectificationsToFinalNotes();
    ...
    if (window.html2pdf) {
      ...
      try {
        await window.html2pdf().set(opt).from(doc).save();
        setStatus("PDF ready");
