...
(() => {
  ...
  function formatDateForPdi(value) {
  ...
  function updateVehicleSummary() {
    const reg = document.querySelector('[name="veh_reg"]')?.value || "";
    const makeModel = document.querySelector('[name="veh_make_model"]')?.value || "";
    const mileage = document.querySelector('[name="veh_mileage_pdi"]')?.value || "";
    const mot = document.querySelector('[name="veh_mot_exp"]')?.value || "";
    const bits = [];
    if (reg) bits.push(reg);
    if (makeModel) bits.push(makeModel);
    if (mileage) bits.push(`${Number(mileage).toLocaleString("en-GB")} miles`);
    if (mot) bits.push(`MOT ${mot}`);
    const text = bits.length ? bits.join("  |  ") : "Vehicle details load automatically from Admin Dashboard.";
    const s1 = document.getElementById("vehicleSummary");
    const s2 = document.getElementById("vehicleSummary2");
    if (s1) s1.textContent = text;
    if (s2) s2.textContent = text;
  }

  function populateVehicleFields(fields) {
    const makeModel = getField(fields, ["Make_Model", "Make & model", "Make & Model", "MakeModel", "Vehicle", "Title"]);
    const reg = getField(fields, ["Registration", "Reg", "VRM"]);
    const vin = getField(fields, ["VIN", "VIN_number", "VIN number", "VIN Number", "VIN / chassis", "VIN / Chassis"]);
    const mileage = getField(fields, ["Mileage", "Mileage at PDI", "Odometer"]);
    const mot = formatDateForPdi(getField(fields, ["MOT_Date", "MOT date", "MOT Date", "MOT expiry", "MOT Expiry", "MOT", "MOT expiry (if known)"]));
    const keys = getField(fields, ["Key_Count", "Keys supplied", "Keys", "Key Count", "Number_of_Keys", "Number of Keys"]);
    ...
    setInputByName("veh_date", isoToday(), { onlyIfEmpty: true });
    setInputByName("veh_inspector", "N Turner", { onlyIfEmpty: true });

    setInputByName("veh_make_model", makeModel, { onlyIfEmpty: false });
    setInputByName("veh_reg", reg, { onlyIfEmpty: false });
    setInputByName("veh_vin", vin, { onlyIfEmpty: false });
    setInputByName("veh_mileage_pdi", mileage, { onlyIfEmpty: false });
    setInputByName("veh_mot_exp", mot, { onlyIfEmpty: false });
    setInputByName("veh_keys", keys, { onlyIfEmpty: false });
    setInputByName("veh_hpi_report_no", hpiReportNo, { onlyIfEmpty: false });
    setInputByName("veh_v5c_no", v5cNo, { onlyIfEmpty: false });
    setInputByName("handover_mileage", mileage, { onlyIfEmpty: true });
  ...
  async function load() {
    try {
      ...
      // Always refresh the core vehicle identity fields from Airtable/Admin Dashboard.
      // Saved tick boxes and notes from PDI_JSON are preserved, but stale vehicle details are corrected.
      populateVehicleFields(fields);
      ...
      document.addEventListener("change", (ev) => {
        ...
        if (t.name && (t.name.startsWith("veh_") || t.name === "handover_mileage")) updateVehicleSummary();
        if (t.name && t.name.endsWith("_note")) validate({ strict: false });
      });

      document.addEventListener("input", (ev) => {
        const t = ev.target;
        if (t?.name && t.name.startsWith("veh_")) updateVehicleSummary();
      });
