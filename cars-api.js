...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "GET" && (path === "/" || path === "")) {
      ...
      if (wantsNoCache) {
        // Safety: only allow nocache when called from your own site/admin.
        // This prevents accidental/public cache bypass from inflating Airtable usage.
        if (!isSameSiteReferrerOrOrigin(request)) {
          return jsonResponse(
            request,
            { ok: false, error: "nocache not allowed from this origin." },
            403,
            { "Cache-Control": "no-store" }
          );
        }
