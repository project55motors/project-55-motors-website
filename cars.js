...
function fmtReg(v) {
  ...
  // Keep letters/numbers/spaces only; collapse multiple spaces
  ...
  // Attempt tasteful UK-style spacing if unspaced (avoids over-formatting private plates)
  const s = cleaned;
...
async function loadCars() {
  ...
  try {
    ...
    records.forEach((rec) => {
      ...
      const mot = fmtMotLong(getField(f, ["MOT_Date"]));
...
if (statusNorm === "sold") {
  ...
  specItem(ICONS.mot, "MOT", mot, "spec--mot", motLong ? `MOT: ${motLong}` : "")
