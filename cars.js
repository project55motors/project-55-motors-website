...
async function loadCars() {
  ...
  try {
    ...
    // Available first, then In Prep, then Arriving Soon, then Sold.
    ...
    const statusPriority = (record) => {
      ...
      if (status === "in_prep" || status === "prep" || status.includes("prep")) return 1;
      if (status === "arriving_soon" || status.includes("arriving")) return 2;
