...
function fmtReg(v) {
  ...
  // If already spaced, keep as-is
  if (cleaned.includes(" ")) return cleaned;

  // Tasteful UK-style spacing for common formats (avoid over-formatting private plates)
...
function specItem(iconUrl, label, value, extraClass = "", titleValue = null) {
  if (!value) return "";
  const tv = titleValue ?? value;
  const icon = iconUrl ? `<img class="spec-icon" src="${iconUrl}" alt="${esc(label)}">` : "";
  const cls = ["spec", extraClass].filter(Boolean).join(" ");
  return `
    <div class="${cls}" aria-label="${esc(label)}: ${esc(tv)}" title="${esc(label)}: ${esc(tv)}">
      ${icon}
...
async function loadCars() {
  ...
  try {
    ...
    records.forEach((rec) => {
      ...
      const motRaw = getField(f, ["MOT_Date"]);
      const motFull = fmtMot(motRaw, "long");
      const motShort = fmtMot(motRaw, "short");
...
if (statusNorm === "sold") {
  ...
  media.appendChild(ribbon);
      ...
      // Icon specs (only show what exists). Premium layout:
      // - 6 items, 2 columns x 3 rows (wider pills = better readability)
      // - Registration + MOT always included when present
      // - MOT displays a short date, with full date in tooltip/aria-label
      const specsHtml = [
        ...
        specItem(ICONS.mot, "MOT", motShort, "spec--mot", motFull)
