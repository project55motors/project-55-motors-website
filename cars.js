...
// Build: 20260110a (Tranche 2: WA robustness + inventory scroll restore)
// HOTFIX 20260109c: resilient vehicle navigation (sessionStorage) + absolute encoded vehicle links
//
// Patch 2026-01-02:
...
document.addEventListener("DOMContentLoaded", () => {
  if (!document.getElementById("car-grid")) return;
  Promise.resolve(loadCars()).then(restoreInventoryScroll).catch(() => {});
});
/* ---------- helpers ---------- */
...
// Save state before navigation (iOS-safe): last vehicle + inventory scroll restore
const saveNavState = () => {
  try {
    ...
    sessionStorage.setItem("p55:inventoryReturn", "1");
    sessionStorage.setItem("p55:inventoryScrollY", String(Math.max(0, window.scrollY || 0)));
  } catch (_) {}
};

// pointerdown fires reliably before navigation on iOS Safari
card.addEventListener("pointerdown", saveNavState, { passive: true });
card.addEventListener("touchstart", saveNavState, { passive: true });
const storeLastVehicle = () => {
  try {
    sessionStorage.setItem("p55:lastVehicleId", String(rec.id || ""));
    sessionStorage.setItem("p55:lastVehicleRec", JSON.stringify({ id: rec.id, fields: rec.fields || {}, __p55: rec.__p55 || null }));
  } catch (_) {}
};
// iOS: pointerdown/touchstart fires earlier than click, improving reliability before navigation
card.addEventListener("pointerdown", storeLastVehicle, { passive: true });
card.addEventListener("touchstart", storeLastVehicle, { passive: true });
card.addEventListener("mousedown", storeLastVehicle);
...
function restoreInventoryScroll() {
  try {
    if (sessionStorage.getItem("p55:inventoryReturn") !== "1") return;
    sessionStorage.removeItem("p55:inventoryReturn");
    const y = parseInt(sessionStorage.getItem("p55:inventoryScrollY") || "0", 10);
    sessionStorage.removeItem("p55:inventoryScrollY");
