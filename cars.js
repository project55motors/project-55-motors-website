...
function ensureStatusStyles() {
  // Additive, non-breaking CSS for new status variants (Prep / Arriving Soon).
  // Does not override existing SOLD ribbon styling.
  if (document.getElementById("p55-status-styles")) return;

  const style = document.createElement("style");
  style.id = "p55-status-styles";
  style.textContent = `
    /* Project 55 Motors — status treatments (additive) */
        .car-card{ position: relative; }
    .car-card[data-status="prep"] .car-image,
    .car-card[data-status="arriving_soon"] .car-image{
      filter: saturate(1.02) contrast(1.02);
    }

    /* Subtle top-left glass chip for non-sold statuses */
    .car-card-status{
      position:absolute;
      top: 12px;
      left: 12px;
    ...
    /* Optional diagonal ribbons for new variants (if your base .car-card-ribbon exists) */
    .car-card-ribbon.prep span{ background: rgba(255,255,255,0.92); color: rgba(15,23,42,0.95); }
    .car-card-ribbon.arriving span{ background: rgba(255,255,255,0.92); color: rgba(15,23,42,0.95); }
    .car-card-ribbon.prep{ opacity: 0.95; }
    .car-card-ribbon.arriving{ opacity: 0.95; }
  `;
...
function canonicalStatus(v) {
  ...
  if (n === "hidden" || n === "hide" || n === "inactive" || n === "draft") return "hidden";
  if (n === "available" || n === "forsale" || n === "for_sale" || n === "live") return "available";
...
// - PREP / ARRIVING SOON get a subtle glass overlay (and optional ribbon variant)
...
const statusCanon = canonicalStatus(statusVal);

if (statusCanon) card.setAttribute("data-status", statusCanon);
...
} else if (statusCanon === "prep") {
  card.classList.add("is-prep");

  // Prefer chip overlay (reads well on all photos)
  card.appendChild(buildStatusOverlay("IN PREP"));
} else if (statusCanon === "arriving_soon") {
