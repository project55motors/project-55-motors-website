...
// Build: 20260726b (Reserved status + consistent display-title formatting)
//
// Patch 2026-01-02:
...
const DISPLAY_TITLE_ACRONYMS = new Set([
  "AMG", "AWD", "BMW", "CDI", "CVT", "DCT", "DSG", "EV", "FSI", "FWD",
  "GT", "GTD", "GTE", "GTI", "HEV", "JCW", "MHEV", "MINI", "MPI",
  "PHEV", "RS", "SE", "SEL", "ST", "SUV", "TDI", "THP", "TSI",
  "TFSI", "ULEZ", "VCT", "VTEC", "VVT", "VW", "WRX"
]);

const DISPLAY_TITLE_STYLES = {
  CRDI: "CRDi",
  DCI: "dCi",
  ECOBOOST: "EcoBoost",
  GDI: "GDi",
...
function fmtVehicleTitle(v) {
  ...
  return raw.split(" ").map((token) => {
    return token.split(/([/-])/).map((part) => {
      ...
      if (DISPLAY_TITLE_STYLES[upper]) {
        return `${prefix}${DISPLAY_TITLE_STYLES[upper]}${suffix}`;
      }

      if (DISPLAY_TITLE_ACRONYMS.has(upper)) {
        return `${prefix}${upper}${suffix}`;
      }

      const isAllCaps = /[A-Z]/.test(core) && core === upper;
      if (!isAllCaps) return part;

      return `${prefix}${upper.charAt(0)}${upper.slice(1).toLowerCase()}${suffix}`;
    }).join("");
  }).join(" ");
}
...
function buildStatusOverlay(label, sublabel = "", theme = "") {
  const wrap = document.createElement("div");
  wrap.className = "car-card-status";

  // Inline styling to guarantee overlay even if CSS is stale
  ...
  chip.style.padding = "8px 11px";
  chip.style.borderRadius = "999px";
  chip.style.color = "rgba(255,255,255,0.97)";
  chip.style.fontWeight = "950";
  chip.style.fontSize = "11px";
  chip.style.letterSpacing = "0.18em";
  chip.style.textTransform = "uppercase";
  chip.style.border = "1px solid rgba(255,255,255,0.18)";
  chip.style.boxShadow = "0 16px 38px rgba(0,0,0,0.40)";
  chip.style.backdropFilter = "blur(10px)";
  chip.style.webkitBackdropFilter = "blur(10px)";

  if (theme === "reserved") {
    chip.style.background = "linear-gradient(135deg, rgba(245,158,11,0.92), rgba(120,53,15,0.82))";
  } else if (theme === "prep") {
    chip.style.background = "linear-gradient(135deg, rgba(26,95,255,0.86), rgba(8,20,38,0.78))";
  } else if (theme === "arriving") {
    chip.style.background = "linear-gradient(135deg, rgba(83,169,255,0.88), rgba(8,20,38,0.78))";
  } else {
    chip.style.background = "linear-gradient(135deg, rgba(8,20,38,0.84), rgba(5,11,24,0.72))";
  }
  ...
  if (sublabel) {
    ...
    sub.style.padding = "7px 11px";
    sub.style.borderRadius = "999px";
    sub.style.background = "rgba(15,23,42,0.42)";
    sub.style.border = "1px solid rgba(255,255,255,0.14)";
    sub.style.color = "rgba(255,255,255,0.86)";
    sub.style.fontWeight = "750";
    sub.style.fontSize = "12px";
    sub.style.letterSpacing = "0.2px";
    sub.style.boxShadow = "0 12px 28px rgba(0,0,0,0.30)";
    sub.style.backdropFilter = "blur(10px)";
    sub.style.webkitBackdropFilter = "blur(10px)";
...
async function loadCars() {
  ...
  try {
    ...
    // Public display priority:
    // Available first, then Reserved, In Prep, Arriving Soon and Sold.
    // JavaScript's stable sort preserves the existing Airtable / Sort_Index
    // order within each status group.
    const statusPriority = (record) => {
      const raw = getField(record?.fields || {}, ["Status"]);
      const value = (raw && typeof raw === "object" && "name" in raw) ? raw.name : raw;
      const status = normStatus(value);

      if (status === "available" || status === "for_sale" || status === "live") return 0;
      if (status === "reserved") return 1;
      if (status === "in_prep" || status === "prep" || status.includes("prep")) return 2;
      if (status === "arriving_soon" || status.includes("arriving")) return 3;
    ...
    const sortedRecords = records
      ...
      .sort((a, b) => statusPriority(a.record) - statusPriority(b.record) || a.index - b.index)
      .map(({ record }) => record);
    ...
    sortedRecords.forEach((rec) => {
      ...
      // Option B: treat empty/hidden status as non-public.
      const _statusNorm0 = normStatus(f.Status);
      if (!_statusNorm0 || _statusNorm0 === "hidden") return;

      const title = fmtVehicleTitle(getField(f, ["Make_Model", "Title", "Vehicle", "Name"]));
      ...
      // Ensure overlays (SOLD ribbon, PREP/ARRIVING chips) anchor correctly
      card.style.position = "relative";
      card.href = `/vehicle?id=${encodeURIComponent(rec.id)}`;

      // Store last-clicked vehicle for resilient navigation (fallback if query strings are lost)
      card.addEventListener("click", () => {
        try {
          sessionStorage.setItem("p55:lastVehicleId", String(rec.id || ""));
          // store a compact record snapshot for offline/error fallback
          sessionStorage.setItem("p55:lastVehicleRec", JSON.stringify({ id: rec.id, fields: rec.fields || {}, __p55: rec.__p55 || null }));
        } catch (_) {}
      });
      ...
      // - RESERVED / IN_PREP / ARRIVING_SOON get a subtle glass “chip” overlay
      ...
      if (statusNorm === "sold") {
      ...
      } else if (statusNorm === "reserved") {
        card.classList.add("is-reserved");
        card.appendChild(buildStatusOverlay("RESERVED", "", "reserved"));
      } else if (statusNorm === "in_prep" || statusNorm === "prep" || statusNorm.includes("prep")) {
        card.classList.add("is-prep");
        card.appendChild(buildStatusOverlay("IN PREP", "", "prep"));
      } else if (statusNorm === "arriving_soon" || statusNorm.includes("arriving")) {
        ...
        card.appendChild(buildStatusOverlay("ARRIVING SOON", etaLabel, "arriving"));
      }
