...
function ensurePremiumStatusStyles() {
  if (document.getElementById("p55-status-pill-styles")) return;

  const style = document.createElement("style");
  style.id = "p55-status-pill-styles";
  style.textContent = `
    /* Premium status overlays (injected by cars.js) */
    ...
    /* Nudge pills down if a ribbon exists, so they don't clash visually */
    .car-card.is-sold .car-card-status { top: 46px; }
...
async function loadCars() {
  ...
  try {
    ...
    records.forEach((rec) => {
      ...
      } else if (statusNorm === "in_prep" || statusNorm === "prep") {
