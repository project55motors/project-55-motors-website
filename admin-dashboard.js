...
const addCarNote = document.getElementById("addCarNote");
...
const focusNewId = new URLSearchParams(window.location.search).get("new");
...
const statusOptions = [{ value: "Available", label: "Available" }, { value: "In_prep", label: "Prep" }, { value: "Arriving_soon", label: "Arriving soon" }, { value: "Sold", label: "Sold" }, { value: "Hidden", label: "Hidden" }];
...
/* Logo (public site shortcut + force fresh stock render) */
adminHomeLogo?.addEventListener("click", async (e) => {
  e.preventDefault();
  try {
    // Best-effort: ask the public stock API to refresh its cache/KV version.
    await fetch("/cars-api?refresh=1", { cache: "no-store" });
  } catch (_) {
    // Ignore — we'll still nocache the page load.
  }
...
function showLogin() {
  ...
  if (addCarNote) addCarNote.style.display = "none";
...
function normaliseStatus(s) {
  // Accept Airtable single-select objects, legacy labels, and friendly UI labels.
  let v = s;
  ...
  if (key === "prep" || key === "in_prep" || key === "inprep") return "In_prep";
  if (key === "arriving_soon" || key === "arrivingsoon") return "Arriving_soon";
  if (key === "sold") return "Sold";
  if (key === "hidden") return "Hidden";
  if (key === "available") return "Available";

  // Fall back to the raw value for any future additions.
  return raw;
}
...
function setStockFilter(next) {
  ...
  renderStockTable();
  if (focusNewId) {
    // Delay a tick so the table is in the DOM
    setTimeout(() => highlightRowById(focusNewId), 50);
  }
}
...
function setAddCarBusy(busy, msg = "Working…") {
  if (!addCarBtn) return;
  addCarBtn.disabled = !!busy;
  if (addCarNote) {
    addCarNote.style.display = "block";
    addCarNote.textContent = msg;
  }
}
...
function highlightRowById(id) {
  ...
  setTimeout(() => tr.classList.remove("p55-new"), 4500);

  // Clean the URL so a refresh doesn't keep re-highlighting
  try {
...
async function addCar() {
  // Create a new record with a sensible default status.
  // If you're filtered, create into that status so the new row is visible immediately.
  const status = (stockFilter && stockFilter !== "All") ? stockFilter : "Hidden";

  setAddCarBusy(true, "Creating new vehicle…");
  try {
    ...
    if (!r.ok || !j?.ok || !j?.id) {
      setAddCarBusy(false, "Create failed.");
      alert(`Add car failed: ${j?.error || "Unknown error"}`);
      return;
    }

    // Reload the page to guarantee fresh state (and highlight the new row).
    const next = `${window.location.pathname}?new=${encodeURIComponent(j.id)}&t=${Date.now()}`;
    window.location.href = next;
  } catch (e) {
    console.error(e);
    setAddCarBusy(false, "Create failed.");
    alert("Add car failed due to a network error.");
  }
}
...
async function loadStock() {
  ...
  let payload = null;
  try {
    const r = await fetch(`${API}/all`, { credentials: "include", cache: "no-store" });
    payload = await r.json().catch(() => null);

    if (!r.ok) {
      const msg = payload?.error || `Failed to load stock (${r.status}).`;
      renderStockError(msg);
      return;
    }

    if (payload && typeof payload === "object" && !Array.isArray(payload) && payload.error) {
      renderStockError(payload.error);
      return;
    }
  } catch (e) {
    console.error(e);
    renderStockError("Failed to load stock (network error).");
    return;
  }
  ...
  if (!cars.length) {
    // Make empty state explicit (avoids “silent failure” if Airtable auth breaks).
    renderEmptyRow(
      "No vehicles returned from Airtable. If this is unexpected, check Airtable credentials/permissions on the admin-worker."
    );
  }
}

function renderStockError(message) {
  const msg = String(message || "Failed to load stock.").trim();

  // Table empty state
  renderEmptyRow(msg);

  // Also give a visible alert so this isn't missed.
  try { alert(msg); } catch {}
}
...
function renderEmptyRow(message) {
  ...
  tr.innerHTML = `
    <td colspan="17" style="padding:14px 14px;">
      <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap;">
        <div style="min-width:0">
          <div style="font-weight:900; letter-spacing:-0.01em;">${escapeHtml(message)}</div>
          <div class="admin-muted" style="margin-top:6px;">
            Tip: in Cloudflare Workers → your admin-worker → Settings → Variables, confirm the Airtable Base ID, Table name and API token are present and correct.
          </div>
        </div>
        <button class="admin-btn ghost" type="button" id="p55RetryStockBtn">Retry</button>
      </div>
    </td>
  `;
  table.appendChild(tr);

  const btn = document.getElementById("p55RetryStockBtn");
  btn?.addEventListener("click", () => loadStock());
}
