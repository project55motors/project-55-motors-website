...
const homeLogo = document.getElementById("adminHomeLogo");
...
const statusOptions = STATUS_OPTIONS.map(o => o.value);
...
homeLogo?.addEventListener("click", (e) => {
  // Force a fresh public view (bypasses edge cache via nocache param).
  e.preventDefault();
...
function normaliseStatus(s) {
  ...
  // Fall back to the original value (keeps compatibility if Airtable introduces new options)
  return raw;
}
...
async function addCar(btn) {
  setRowBusy(btn, true, "Adding…");

  try {
    // Default to current filter (if specific) so it appears immediately; otherwise keep it Hidden to avoid accidental public display.
    const defaultStatus = (stockFilter && stockFilter !== "All") ? stockFilter : "Hidden";
    ...
    const out = await res.json().catch(() => ({}));
    if (!res.ok) {
      alert(`Add car failed: ${out.error || res.statusText || "Unknown error"}`);
      return;
    }

    // Expect { id } or Airtable-like { id, fields }
    lastCreatedId = out.id || out.record?.id || null;

    // Ensure it is visible even if you were in a narrower filter.
    if (stockFilter !== "All") {
      stockFilter = "All";
      syncStockFilterUI();
    }
...
function renderStockTable() {
  ...
  cars.forEach(c => {
    ...
    tr.dataset.id = c.id;
    if (lastCreatedId && c.id === lastCreatedId) tr.classList.add("admin-row-new");
    tr.innerHTML = `
...
// If we just created a record, bring it into view and focus the first cell for immediate editing.
if (lastCreatedId) {
  try {
    const esc = (window.CSS && CSS.escape) ? CSS.escape(lastCreatedId) : String(lastCreatedId).replace(/[^a-zA-Z0-9_-]/g, "\\$&");
    const row = table.querySelector(`tr[data-id="${esc}"]`);
    if (row) {
      row.scrollIntoView({ behavior: "smooth", block: "center" });
      const focusEl = row.querySelector("input, textarea, select");
      focusEl?.focus?.();
    }
  } catch (e) {
    // no-op
