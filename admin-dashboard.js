...
function renderStockTable() {
  ...
  cars.forEach(c => {
    ...
    tr.innerHTML = `
      ...
      <td><input class="admin-cell-input" value="${escapeHtml((c.VIN ?? c['VIN number'] ?? c['VIN Number'] ?? '') || '')}"></td>
      <td><input class="admin-cell-input" value="${escapeHtml((c['V5C number'] ?? c['V5C Number'] ?? c.V5C ?? '') || '')}"></td>
      <td><input class="admin-cell-input" value="${escapeHtml((c['HPI report number'] ?? c['HPI Report number'] ?? c['HPI Report Number'] ?? c['HPI report no'] ?? '') || '')}"></td>
...
async function save(id, btn) {
  ...
  const core = {
    ...
    Short_Description: txt(cellValue(tds[13])),
    Full_Description: txt(cellValue(tds[14]))
  };

  // Treat empty strings as null for text-ish fields
  for (const k of [
    "Fuel_type", "Transmission", "Status",
    "Short_Description", "Full_Description",
    "Make_Model", "Registration", "MOT_Date"
  ]) {
    if (core[k] === "") core[k] = null;
  }

  const VIN_KEYS = ["VIN", "VIN number", "VIN Number"];
  const V5C_KEYS = ["V5C number", "V5C Number", "V5C"];
  const HPI_KEYS = ["HPI report number", "HPI Report number", "HPI Report Number", "HPI report no", "HPI Report No"];

  let vinIx = 0, v5cIx = 0, hpiIx = 0;

  const buildFields = () => {
    const fields = { ...core };

    // Only set these if there is a value (or explicitly clear if empty)
    fields[VIN_KEYS[vinIx]] = vinVal === "" ? null : vinVal;
    fields[V5C_KEYS[v5cIx]] = v5cVal === "" ? null : v5cVal;
    fields[HPI_KEYS[hpiIx]] = hpiVal === "" ? null : hpiVal;

    return fields;
  };

  const postUpdate = async (fields) => {
    const res = await fetch(`/api/update/${id}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  // Attempt update; if Airtable rejects unknown field names, retry with alternative names.
  let fields = buildFields();
  for (let attempt = 0; attempt < 3; attempt++) {
    const { res, out } = await postUpdate(fields);
    ...
    const msg = String(out?.error || out?.detail?.error?.message || "");
    ...
    if (unknownList.includes(VIN_KEYS[vinIx]) && vinIx < VIN_KEYS.length - 1) { vinIx++; changed = true; }
    if (unknownList.includes(V5C_KEYS[v5cIx]) && v5cIx < V5C_KEYS.length - 1) { v5cIx++; changed = true; }
    if (unknownList.includes(HPI_KEYS[hpiIx]) && hpiIx < HPI_KEYS.length - 1) { hpiIx++; changed = true; }
  ...
  alert("Save failed: field mapping could not be resolved.");
}
