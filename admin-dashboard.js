...
// ---------- Table wiring (Save button enablement) ----------
// The Worker returns pre-rendered <tr> rows that often ship with Save buttons disabled.
// We must re-enable Save buttons when the user edits any field, otherwise "Save" appears
// inactive and no network request will ever fire.

function findSaveButton(row) {
  if (!row) return null;

  // Most common patterns (keep robust as the Worker HTML has changed over time)
  return (
...
function wireSaveButtons() {
  ...
  tbody.querySelectorAll('tr[data-id]').forEach((row) => {
    // Ensure thumbnails remain constrained, even if the Worker HTML omitted a class.
    ...
    // Ensure disabled until something changes
...
async function loadCars() {
  ...
  try {
    ...
    if (tableBody) {
      ...
      // The Worker delivers pre-rendered rows; once injected we must re-wire
      // client-side behaviour (e.g., enabling Save on field edits).
      wireSaveButtons();
    }
...
async function save(id) {
  ...
  try {
    ...
    // After a successful save, treat the row as clean.
    const saveBtn = findSaveButton(row);
