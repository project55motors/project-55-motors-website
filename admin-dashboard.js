...
const editMarkSoldBtn = document.getElementById("editMarkSoldBtn");
const editRevertBtn = document.getElementById("editRevertBtn");
...
const editRegistration = document.getElementById("editRegistration");
const editPrice = document.getElementById("editPrice");
const editMileage = document.getElementById("editMileage");
const editMotDate = document.getElementById("editMotDate");
...
const editShortDesc = document.getElementById("editShortDesc");
const editFullDesc = document.getElementById("editFullDesc");
...
function toast(kind, message, ms = 2600) {
  if (!toastHost) return;
  const el = document.createElement("div");
  el.className = `admin-toast ${kind ? `admin-toast--${kind}` : ""}`.trim();
...
function setBusy(btn, busy, label = "Saving…") {
  if (!btn) return;
  btn.disabled = !!busy;
  if (busy) {
...
async function quickStatusUpdate(id, nextStatus, btn) {
  ...
  try {
    ...
    if (!res.ok) {
      toast("error", `Update failed: ${out.error || "Unknown error"}`);
      return false;
    }
    ...
    console.error(err);
    toast("error","Update failed due to a network error.");
    return false;
  } finally {
...
function openEdit(id) {
  ...
  if (!car || !editOverlay) return;

  currentEditId = id;

  const status = normaliseStatus(car.Status);
  editTitle.textContent = car.Make_Model || "Vehicle";
  editSub.textContent = car.Registration ? `Registration: ${car.Registration}` : "—";

  fillSelect(editFuel, fuelOptions, car.Fuel_type || "Petrol");
  fillSelect(editTransmission, transmissionOptions, car.Transmission || "Manual");
  fillSelect(editStatus, statusOptions, status || "Available");

  editMakeModel.value = car.Make_Model || "";
  editRegistration.value = car.Registration || "";
  editPrice.value = car.Price ?? "";
  editMileage.value = car.Mileage ?? "";
  editMotDate.value = car.MOT_Date || "";
  editEngine.value = car.Engine_size ?? "";
  editShortDesc.value = car.Short_Description || "";
  editFullDesc.value = car.Full_Description || "";

  editSaveNote.textContent = "Tap “Save changes” when you’re done.";
  editSaveNote.classList.remove("admin-note--error", "admin-note--success");

  editOverlay.style.display = "flex";
  document.body.classList.add("admin-modal-open");
  setTimeout(() => editMakeModel?.focus?.(), 50);
}
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape" && editOverlay?.style?.display === "flex") closeEdit();
});

editMarkSoldBtn?.addEventListener("click", async () => {
  if (!currentEditId) return;
  try {
    setBusy(editMarkSoldBtn, true, "Marking…");
    await saveFields(currentEditId, { Status: "Sold" });
    updateLocalCar(currentEditId, { Status: "Sold" });
    renderStockTable();
    toast("success", "Marked as Sold.");
    fillSelect(editStatus, statusOptions, "Sold");
  } catch (err) {
    toast("error", `Update failed: ${err.message || "Unknown error"}`);
  } finally {
    setBusy(editMarkSoldBtn, false);
  }
});

editRevertBtn?.addEventListener("click", async () => {
  if (!currentEditId) return;
  try {
    setBusy(editRevertBtn, true, "Reverting…");
    await saveFields(currentEditId, { Status: "Available" });
    updateLocalCar(currentEditId, { Status: "Available" });
    renderStockTable();
    toast("success", "Reverted to Available.");
    fillSelect(editStatus, statusOptions, "Available");
  } catch (err) {
    toast("error", `Update failed: ${err.message || "Unknown error"}`);
  } finally {
    setBusy(editRevertBtn, false);
  }
});
...
editSaveBtn?.addEventListener("click", async () => {
  ...
  const fields = {
    Make_Model: txt(editMakeModel?.value || ""),
    Registration: txt(editRegistration?.value || ""),
    Price: num(editPrice?.value || ""),
    Mileage: num(editMileage?.value || ""),
    MOT_Date: txt(editMotDate?.value || ""),
    Engine_size: num(editEngine?.value || ""),
    Fuel_type: txt(editFuel?.value || ""),
    Transmission: txt(editTransmission?.value || ""),
    Status: txt(editStatus?.value || ""),
    Short_Description: txt(editShortDesc?.value || ""),
    Full_Description: txt(editFullDesc?.value || "")
  };

  try {
    setBusy(editSaveBtn, true, "Saving…");
    await saveFields(currentEditId, fields);
    updateLocalCar(currentEditId, fields);
    renderStockTable();

    editSaveNote.textContent = "Saved successfully.";
    editSaveNote.classList.remove("admin-note--error");
    editSaveNote.classList.add("admin-note--success");

    toast("success", "Saved.");
  } catch (err) {
    console.error(err);
    editSaveNote.textContent = `Save failed: ${err.message || "Unknown error"}`;
    editSaveNote.classList.remove("admin-note--success");
    editSaveNote.classList.add("admin-note--error");
...
async function loadStock() {
  ...
  if (!r.ok) toast("error","Failed to load stock."); return;
...
function renderStockTable() {
  ...
  cars.forEach(c => {
    ...
    tr.innerHTML = `
      ...
      <td><div class="admin-chip">${escapeHtml(make || "—")}</div></td>
      <td><div class="admin-chip admin-chip--mono">${escapeHtml(reg || "—")}</div></td>
      <td><div class="admin-chip">${c.Price != null ? "£" + fmt.format(Number(c.Price)) : "—"}</div></td>
      <td><div class="admin-chip">${c.Mileage != null ? fmt.format(Number(c.Mileage)) : "—"}</div></td>
      <td><div class="admin-chip">${escapeHtml(c.MOT_Date || "—")}</div></td>
      <td><div class="admin-chip">${c.Engine_size != null ? escapeHtml(String(c.Engine_size)) : "—"}</div></td>
      <td><div class="admin-chip">${escapeHtml(c.Fuel_type || "—")}</div></td>
      <td><div class="admin-chip">${escapeHtml(c.Transmission || "—")}</div></td>
      <td><div class="admin-chip ${status === "Sold" ? "admin-chip--sold" : (status === "Hidden" ? "admin-chip--hidden" : "admin-chip--live")}">${escapeHtml(status || "—")}</div></td>

      <td><div class="admin-preview">${escapeHtml(clampText(c.Short_Description, 90) || "—")}</div></td>
      <td><div class="admin-preview">${escapeHtml(clampText(c.Full_Description, 110) || "—")}</div></td>

      <td>
        <div style="display:flex; gap:8px; flex-wrap:wrap; justify-content:flex-end;">
          <button class="admin-btn tiny primary" type="button" onclick="openEdit('${c.id}')">Edit</button>
          <button class="admin-btn tiny ghost" type="button" onclick="markSold('${c.id}', this)">Mark sold</button>
          <button class="admin-btn tiny ghost" type="button" onclick="markAvailable('${c.id}', this)">Revert</button>
        </div>
      </td>
    `;
    ...
    tr.addEventListener("click", (e) => {
      ...
      if (target && (target.closest("button") || target.closest("a") || target.closest("input") || target.closest("select") || target.closest("textarea"))) return;
      openEdit(c.id);
    });
