...
const aiCopyFullBtn = document.getElementById("aiCopyFullBtn");
const aiCopyShortBtn = document.getElementById("aiCopyShortBtn");
...
const aiFullDescription = document.getElementById("aiFullDescription");
const aiShortDescription = document.getElementById("aiShortDescription");
const aiCazooDescription = document.getElementById("aiCazooDescription");
const aiFacebookPost = document.getElementById("aiFacebookPost");
const aiRiskNotes = document.getElementById("aiRiskNotes");


let activeDescTarget = null; // table textarea currently being edited
let activeAIReview = null; // currently reviewed AI advert
...
function getAiReviewValues() {
  return {
    short_description: aiShortDescription?.value || "",
    full_description: aiFullDescription?.value || "",
    cazoo_description: aiCazooDescription?.value || "",
    facebook_post: aiFacebookPost?.value || "",
    risk_notes: aiRiskNotes?.value || ""
  };
}

function openAiReview(id, generated = {}) {
  const car = (lastCars || []).find(c => c.id === id) || {};
  activeAIReview = { id, generated };

  if (aiReviewTitle) {
    const label = [car.Registration, car.Make_Model].filter(Boolean).join(" · ");
    aiReviewTitle.textContent = label ? `AI advert review — ${label}` : "AI advert review";
  }

  if (aiShortDescription) {
    aiShortDescription.value =
      generated.AI_Website_Short_Description ||
      generated.website_short_description ||
      car.AI_Website_Short_Description ||
      "";
  }

  if (aiFullDescription) {
    aiFullDescription.value =
      generated.AI_Website_Full_Description ||
      generated.website_full_description ||
      car.AI_Website_Full_Description ||
      "";
  }

  if (aiCazooDescription) {
    aiCazooDescription.value =
      generated.AI_Motors_Description ||
      generated.motors_description ||
      car.AI_Motors_Description ||
      "";
  }

  if (aiFacebookPost) {
    aiFacebookPost.value =
      generated.AI_Facebook_Post ||
      generated.facebook_post ||
      car.AI_Facebook_Post ||
      "";
  }

  if (aiRiskNotes) {
    aiRiskNotes.value =
      generated.AI_Risk_Notes ||
  ...
  if (aiReviewStatus) {
    aiReviewStatus.textContent = "Review the AI output, edit the website copy if needed, then approve it into the live advert.";
  }

  setAiReviewOpen(true);
  setTimeout(() => aiFullDescription?.focus(), 0);
}

async function approveAiReview() {
  if (!activeAIReview?.id) return;

  const id = activeAIReview.id;
  const values = getAiReviewValues();

  if (!values.short_description.trim() && !values.full_description.trim()) {
    alert("There is no website description to approve.");
    return;
  }

  const ok = confirm(
    "Approve this AI advert?\n\nThis will copy the edited website short and full descriptions into the live website advert fields."
  );
  if (!ok) return;
  ...
  if (aiReviewStatus) aiReviewStatus.textContent = "Approving edited website advert…";

  try {
    const res = await fetch(`/api/ai/approve/${encodeURIComponent(id)}`, {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        cazoo_description: values.cazoo_description,
    ...
    if (!res.ok || !out.ok) {
      ...
      if (aiReviewStatus) aiReviewStatus.textContent = "Approve failed. Please check the error and try again.";
      return;
    }
    ...
    updateLocalCar(id, {
      ...
      Short_Description: values.short_description,
      Full_Description: values.full_description,
      AI_Website_Short_Description: values.short_description,
      AI_Website_Full_Description: values.full_description,
      AI_Motors_Description: values.cazoo_description,
      AI_Facebook_Post: values.facebook_post,
      AI_Approved: true
    });
    ...
    renderStockTable();
    closeAiReview();
    alert("AI advert approved and copied into the live website description fields.");
  } catch (err) {
    console.error(err);
    alert("Approve failed due to a network error.");
    if (aiReviewStatus) aiReviewStatus.textContent = "Approve failed due to a network error.";
  } finally {
    if (aiApproveBtn) {
      aiApproveBtn.disabled = false;
      aiApproveBtn.textContent = aiApproveBtn.dataset.prevText || "Approve edited website advert";
...
async function generateAI(id, btn) {
  ...
  setRowBusy(btn, true, "Researching…");

  try {
    // First refresh AI_Model_Research so the advert generation has the latest model context.
    ...
    updateLocalCar(id, {
      AI_Model_Research: researchOut.research || ""
    });
...
window.approveAiReview = approveAiReview;
...
async function copyAiReviewText(kind) {
  const values = getAiReviewValues();
  const map = {
    full: ["Website full description", values.full_description],
    short: ["Website short description", values.short_description],
    cazoo: ["Cazoo description", values.cazoo_description],
    facebook: ["Facebook post", values.facebook_post]
  };

  const [label, value] = map[kind] || ["Text", ""];
  ...
  try {
    ...
    alert(ok ? `${label} copied to clipboard.` : `Could not copy ${label}.`);
  } catch (err) {
    console.error(err);
...
aiApproveBtn?.addEventListener("click", approveAiReview);
aiCopyFullBtn?.addEventListener("click", () => copyAiReviewText("full"));
aiCopyShortBtn?.addEventListener("click", () => copyAiReviewText("short"));
aiCopyCazooBtn?.addEventListener("click", () => copyAiReviewText("cazoo"));
aiCopyFacebookBtn?.addEventListener("click", () => copyAiReviewText("facebook"));
...
document.addEventListener("keydown", (e) => {
  if (!aiReviewOverlay || aiReviewOverlay.style.display !== "flex") return;
