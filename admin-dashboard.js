...
const statusOptions = ["Available", "In_prep", "Arriving_soon", "Sold", "Hidden"];
...
/* ---------- Home logo (force fresh public stock) ---------- */
const adminHomeLogo = document.getElementById("adminHomeLogo") || document.getElementById("adminHomeLogoLink");
if (adminHomeLogo) {
  adminHomeLogo.addEventListener("click", async (e) => {
    // Always force public stock refresh before jumping to the public site.
    e.preventDefault();
    try {
      await fetch("/cars-api?refresh=1", { cache: "no-store" });
    } catch (_) {}
    const ts = Date.now();
    window.location.href = `/index.html?nocache=1&ts=${ts}`;
  });
}
...
function renderStockTable() {
  ...
  cars.forEach(c => {
    ...
    tr.innerHTML = `
      ...
      <td><input class="admin-cell-input col-v5c" value="${escapeHtml((c.V5C_number ?? c.V5C ?? c['V5C number'] ?? c['V5C Number'] ?? '') || '')}"></td>
      <td><input class="admin-cell-input col-hpi" value="${escapeHtml((c.HPI_report_number ?? c['HPI report number'] ?? c['HPI Report Number'] ?? c['HPI report no'] ?? '') || '')}"></td>
