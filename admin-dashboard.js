...
const aiReviewSub = document.getElementById("aiReviewSub");
...
const aiApproveFromReviewBtn = document.getElementById("aiApproveFromReviewBtn");
...
const aiShortCount = document.getElementById("aiShortCount");
const aiFullCount = document.getElementById("aiFullCount");
const aiReviewNote = document.getElementById("aiReviewNote");
...
function updateAiReviewCounts() {
  if (aiShortCount && aiReviewShort) aiShortCount.textContent = `${fmt.format(aiReviewShort.value.length)} characters`;
  if (aiFullCount && aiReviewFull) aiFullCount.textContent = `${fmt.format(aiReviewFull.value.length)} characters`;
}

function currentAiReviewPayload() {
  return {
    short_description: aiReviewShort?.value?.trim() || "",
    full_description: aiReviewFull?.value?.trim() || "",
    motors_description: aiReviewCazoo?.value?.trim() || "",
    facebook_post: aiReviewFacebook?.value?.trim() || "",
    risk_notes: aiReviewRisk?.value?.trim() || ""
  };
}

function openAiReview(carOrId) {
  const car = typeof carOrId === "string"
    ? (lastCars || []).find(c => c.id === carOrId)
    : carOrId;

  if (!car?.id) {
    alert("Could not open AI review for this vehicle.");
    return;
  }

  activeAiReviewId = car.id;

  if (aiReviewTitle) aiReviewTitle.textContent = `AI Review — ${car.Registration || "Vehicle"}`;
  if (aiReviewSub) aiReviewSub.textContent = car.Make_Model || "Review and edit the AI output before approving it.";

  if (aiReviewShort) aiReviewShort.value = car.AI_Website_Short_Description || "";
  if (aiReviewFull) aiReviewFull.value = car.AI_Website_Full_Description || "";
  if (aiReviewCazoo) aiReviewCazoo.value = car.AI_Motors_Description || "";
  if (aiReviewFacebook) aiReviewFacebook.value = car.AI_Facebook_Post || "";
  if (aiReviewRisk) aiReviewRisk.value = car.AI_Risk_Notes || "";
  if (aiReviewNote) aiReviewNote.textContent = "Edit anything you want, then Approve AI to copy the website descriptions into the live fields.";

  updateAiReviewCounts();
...
aiReviewShort?.addEventListener("input", updateAiReviewCounts);
aiReviewFull?.addEventListener("input", updateAiReviewCounts);
aiReviewCloseBtn?.addEventListener("click", closeAiReview);
aiReviewCancelBtn?.addEventListener("click", closeAiReview);
aiReviewOverlay?.addEventListener("click", (e) => { if (e.target === aiReviewOverlay) closeAiReview(); });

aiCopyCazooBtn?.addEventListener("click", async () => {
  const ok = await copyTextToClipboard(aiReviewCazoo?.value || "");
  alert(ok ? "Cazoo advert copied." : "Cazoo advert is empty or could not be copied.");
});

aiCopyFacebookBtn?.addEventListener("click", async () => {
  const ok = await copyTextToClipboard(aiReviewFacebook?.value || "");
  alert(ok ? "Facebook post copied." : "Facebook post is empty or could not be copied.");
});

aiApproveFromReviewBtn?.addEventListener("click", async () => {
  if (!activeAiReviewId) return;
  await approveAI(activeAiReviewId, aiApproveFromReviewBtn, currentAiReviewPayload());
});

async function runModelResearch(id) {
  const res = await fetch(`/api/ai/research/${encodeURIComponent(id)}`, { method: "POST", credentials: "include" });
  const out = await res.json().catch(() => ({}));
  if (!res.ok || !out.ok) throw new Error(out.error || "Research failed");
  updateLocalCar(id, { AI_Model_Research: out.research || "" });
  return out;
}
...
async function generateAI(id, btn) {
  ...
  const hasExistingAi = !!String(car?.AI_Website_Full_Description || "").trim();

  if (hasExistingAi) {
    const regenerate = confirm("AI content already exists for this vehicle.\n\nOK = regenerate AI content\nCancel = review the existing AI content");
    if (!regenerate) {
      openAiReview(car);
      return;
    }
  } else {
    const ok = confirm("Generate AI advert content for this vehicle? This will run model research first, then generate the AI advert fields.");
    if (!ok) return;
  }
  ...
  try {
    ...
    updateLocalCar(id, { ...generated, AI_Status: "Generated", AI_Last_Generated: new Date().toISOString(), AI_Approved: false });
    const updatedCar = (lastCars || []).find(c => c.id === id);
    openAiReview(updatedCar || { id, ...generated });
  } catch (err) {
    console.error(err);
    alert(err.message || "AI workflow failed due to a network error.");
  } finally {
    setRowBusy(btn, false);
  }
}
...
async function approveAI(id, btn, editedPayload = null) {
  if (!id) return;
  const payload = editedPayload || currentAiReviewPayload();

  if (!payload.full_description) {
    alert("Full description is empty. Add or generate AI content before approving.");
    return;
  }

  const ok = confirm("Approve this AI website advert?\n\nThis will copy the edited AI short and full descriptions into the live Short Description and Full Description fields.");
  if (!ok) return;
  ...
  try {
    ...
    updateLocalCar(id, { ...(out.approved || {}), AI_Approved: true });
    refreshPublicStockCache();
    closeAiReview();
    renderStockTable();
