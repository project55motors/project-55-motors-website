...
async function researchAI(id, btn) {
  if (!id) return;

  const ok = confirm("Generate model research for this vehicle? This updates AI_Model_Research in Airtable.");
  if (!ok) return;

  setRowBusy(btn, true, "Researching…");

  try {
    const res = await fetch(`/api/ai/research/${encodeURIComponent(id)}`, {
      method: "POST",
      credentials: "include"
    });
    ...
    if (!res.ok || !out.ok) {
      alert(`Research failed: ${out.error || "Unknown error"}`);
      return;
    }

    updateLocalCar(id, {
      AI_Model_Research: out.research || ""
    });

    alert("Model research saved to Airtable.");
  } catch (err) {
    console.error(err);
    alert("Research failed due to a network error.");
  } finally {
    setRowBusy(btn, false);
  }
}

window.researchAI = researchAI;

async function approveAI(id, btn) {
  if (!id) return;

  const ok = confirm(
    "Approve AI website advert?\n\nThis will copy the AI short and full website descriptions into the live Short Description and Full Description fields."
  );
  if (!ok) return;
...
async function copyAiField(id, fieldName, label, btn) {
  const car = (lastCars || []).find(c => c.id === id);
  const value = car?.[fieldName];

  if (!String(value || "").trim()) {
    alert(`${label} is empty. Generate AI content first.`);
    return;
  }

  setRowBusy(btn, true, "Copying…");
    ...
    tr.innerHTML = `      <td class="col-photo">${img ? `<img class="admin-thumb" src="${escapeHtml(img)}" alt="">` : ""}</td>
      ...
      <td class="col-actions">
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          ...
          <button class="admin-btn tiny ghost" onclick="researchAI('${c.id}', this)">Research</button>
          <button class="admin-btn tiny ghost" onclick="generateAI('${c.id}', this)">AI</button>
          <button class="admin-btn tiny ghost" onclick="approveAI('${c.id}', this)">Approve AI</button>
          <button class="admin-btn tiny ghost" onclick="copyAiField('${c.id}', 'AI_Motors_Description', 'Cazoo advert', this)">Cazoo</button>
          <button class="admin-btn tiny ghost" onclick="copyAiField('${c.id}', 'AI_Facebook_Post', 'Facebook post', this)">Facebook</button>
