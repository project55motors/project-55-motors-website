...
async function refreshPublicStock() {
  try {
    // Bump the versioned cache in cars-api so the public site updates immediately.
    await fetch("/cars-api?refresh=1", { cache: "no-store" });
  } catch (e) {
    console.warn("Public stock refresh failed (non-fatal):", e);
  }
}
...
// Home logo: refresh public stock cache then open website
if (homeLogo) {
...
function canonicalStatusValue(raw) {
  const s = String(raw || "").trim();
  const norm = s.toLowerCase().replace(/\s+/g, "_");
  if (!norm) return "Available";
  if (norm === "prep" || norm === "in_prep" || norm === "inprep") return "In_prep";
  if (norm === "arriving_soon" || norm === "arrivingsoon") return "Arriving_soon";
  if (norm === "sold") return "Sold";
  if (norm === "hidden") return "Hidden";
  return (s === "Available") ? "Available" : (s === "Sold") ? "Sold" : (s === "Hidden") ? "Hidden" : "Available";
}
...
function renderStockTable() {
  ...
  cars.forEach(c => {
    ...
    tr.innerHTML = `
      <td class="col-photo">${img ? `<img class="admin-thumb" src="${escapeHtml(img)}" alt="">` : ""}</td>

      <td class="col-make"><input class="admin-cell-input" value="${escapeHtml(c.Make_Model || "")}"></td>
      <td class="col-reg"><input class="admin-cell-input" value="${escapeHtml(c.Registration || "")}"></td>

      <td class="col-vin"><input class="admin-cell-input col-vin" value="${escapeHtml((c.VIN ?? c.VIN_number ?? c['VIN number'] ?? c['VIN Number'] ?? '') || '')}"></td>
      <td class="col-v5c"><input class="admin-cell-input" value="${escapeHtml((c.V5C_number ?? c.V5C ?? c['V5C number'] ?? c['V5C Number'] ?? '') || '')}"></td>
      <td class="col-hpi"><input class="admin-cell-input" value="${escapeHtml((c.HPI_report_number ?? c['HPI report number'] ?? c['HPI Report Number'] ?? c['HPI report no'] ?? '') || '')}"></td>
      <td class="col-price"><input class="admin-cell-input" type="number" value="${c.Price ?? ""}"></td>
      <td class="col-mileage"><input class="admin-cell-input col-mileage" type="number" value="${c.Mileage ?? ""}"></td>
      <td class="col-mot"><input class="admin-cell-input" type="date" value="${escapeHtml(c.MOT_Date || "")}"></td>
      ...
      <td class="col-status">
        <select class="admin-cell-select col-status">
          ${statusOptions.map(opt => `<option value="${opt.value}" ${canonicalStatusValue(c.Status) === opt.value ? "selected" : ""}>${opt.label}</option>`).join("")}
        </select>
      </td>
