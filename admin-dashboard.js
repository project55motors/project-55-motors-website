...
/**
 * Debounced public-stock refresh.
 * - Coalesces multiple admin edits into a single refresh call (reduces Airtable/API load).
 * - Default delay is 2500ms which is short enough to feel "immediate" without hammering.
 */
function schedulePublicStockRefresh(delayMs = 2500) {
  try {
    if (__p55RefreshTimer) clearTimeout(__p55RefreshTimer);
    __p55RefreshTimer = setTimeout(() => {
      __p55RefreshTimer = null;
      schedulePublicStockRefresh();
...
/**
 * Warms cars-api with a no-cache read (optional).
 * This helps ensure the public site reflects the latest changes immediately after admin edits.
 */
function refreshPublicStockCache() {
  ...
  __p55RefreshInFlight = (async () => {
    try {
      // Warm the public stock endpoint (bypasses edge cache).
