    ...
    try {
        ...
        photoSection.innerHTML = `
            <div id="main-photo-container">
                <img id="main-car-photo" src="${mainPhotoUrl}" alt="${model} main photo">
            </div>
            <div id="thumbnail-gallery">
                ${photos.map((p, i) => `
                    <div class="thumbnail ${i === 0 ? 'active' : ''}" data-url="${p.url}">
                        <img src="${p.url}" alt="${model} thumbnail ${i + 1}" loading="lazy">
                    </div>
                `).join('')}
            </div>
        `;

        contentContainer.appendChild(photoSection);
        ...
        contentContainer.appendChild(specsDiv);

        // Thumbnail click handling
        document.querySelectorAll('.thumbnail').forEach(thumbnail => {
            thumbnail.addEventListener('click', function() {
                const mainPhoto = document.getElementById('main-car-photo');
                mainPhoto.src = this.getAttribute('data-url');
