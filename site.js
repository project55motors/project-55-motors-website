...
/* ---------- Footer cleanup: remove "No distance selling" line everywhere ---------- */
function removeNoDistanceSellingLine(root=document){
  const needles = [/no distance selling/i, /more info in terms/i];
  const candidates = [];
  // Prefer footer only, but fall back to whole doc
  const footer = root.querySelector("#siteFooter") || root.querySelector("footer") || root;
  candidates.push(footer);

  for (const scope of candidates){
    if (!scope) continue;

    // Remove whole elements whose text content matches
    const els = Array.from(scope.querySelectorAll("p,div,span,li,a,small"));
    for (const el of els){
      const t = (el.textContent || "").trim();
      if (!t) continue;
      if (needles.some(rx => rx.test(t))){
        // If it's an anchor that also contains other needed text, only strip the phrase.
        if (el.childNodes && el.childNodes.length && !["P","DIV","LI","SMALL"].includes(el.tagName)){
          el.textContent = t
            .replace(/no distance selling\.?/ig, "")
            .replace(/more info in terms\.?/ig, "")
            .replace(/\s{2,}/g, " ")
            .trim();
    ...
    // Also strip matching text nodes (rare but safe)
    const walker = document.createTreeWalker(scope, NodeFilter.SHOW_TEXT, null);
    const toClean = [];
    while (walker.nextNode()) {
      const node = walker.currentNode;
      const txt = (node.nodeValue || "");
      if (needles.some(rx => rx.test(txt))) toClean.push(node);
    }
    for (const n of toClean){
      n.nodeValue = (n.nodeValue || "")
        .replace(/no distance selling\.?\s*/ig, "")
        .replace(/more info in terms\.?\s*/ig, "")
        .replace(/\s{2,}/g, " ");
      if (!(n.nodeValue || "").trim()){
...
(function () {
  ...
  if (p.includes("admin") || p.includes("pdi")) return; // never run on admin pages
  ...
  function initWhatsAppFab() {
    // Only show the floating WhatsApp button on vehicle detail pages.
  ...
  function injectFooterLegal() {
    // Public pages only (do not inject into admin/PDI pages)
...
try { removeNoDistanceSellingLine(); } catch(_) {}
