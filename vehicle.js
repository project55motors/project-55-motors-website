...
const P55_VEHICLE_UI_VERSION = "v6-2026-01-01";
...
function p55PruneInlineWhatsAppUI() {
  // Remove/Hide inline WhatsApp rows (keep only the floating #whatsapp-btn)
  const keepFab = document.getElementById("whatsapp-btn");

  // Hide any WhatsApp links inside the page that are not the FAB
  document.querySelectorAll('#vehicle-page a[href*="wa.me"], #vehicle-page a[href*="whatsapp.com"], #vehicle-page a[href^="whatsapp:"]').forEach(a => {
    if (keepFab && a === keepFab) return;
    const row = a.closest("p, div, li, section") || a;
    row.classList.add("p55-legacy-row");
    row.setAttribute("aria-hidden", "true");
  });

  // Hide helper text even if not wrapped in a WA link (browser differences)
  document.querySelectorAll("#vehicle-page p, #vehicle-page div, #vehicle-page span, #vehicle-page small").forEach(el => {
    const t = (el.textContent || "").trim().toLowerCase();
    if (!t) return;

    if (
      t.includes("message about this car") ||
      t.includes("quick question") ||
      t.includes("viewing times") ||
      t.includes("part-exchange")
    ) {
      // Avoid hiding the main enquiry CTA container
