(function(){
'use strict';
if (window.__P55_VEHICLE_JS_LOADED__) return;
window.__P55_VEHICLE_JS_LOADED__ = true;
...
const P55_VEHICLE_UI_VERSION = "v7.0-2026-01-01";
...
function p55InitVehicle() {
  // Avoid duplicate in-flight initialisations (e.g. async script + pageshow)
  if (window.__p55VehicleInitInFlight) return;
  window.__p55VehicleInitInFlight = true;

  Promise.resolve()
    .then(() => loadVehicle())
    .catch((e) => {
      console.error("Vehicle init failed:", e);
      // If the script was loaded late (async) and DOM wasn't ready, retry shortly.
      if (!window.__p55VehicleRendered) {
        setTimeout(() => {
          try { loadVehicle(); } catch (_) {}
        }, 250);
      }
    })
    .finally(() => { window.__p55VehicleInitInFlight = false; });
}

function p55Boot(retry = 0) {
  // If already rendered, do nothing
  if (window.__p55VehicleRendered) return;

  // If key placeholders are not in DOM yet (async script loaded early), retry briefly
  const hasHero = !!document.getElementById("vehicle-hero");
  const hasTitle = !!document.getElementById("vehicle-title");
  if ((!hasHero && !hasTitle) && retry < 30) {
    setTimeout(() => p55Boot(retry + 1), 100);
    return;
  }

  p55InitVehicle();

  // If we still haven't rendered after a second, try again a few times
  if (retry < 5) {
    setTimeout(() => {
      if (!window.__p55VehicleRendered) p55Boot(retry + 1);
    }, 1000);
  }
}
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => p55Boot(0));
} else {
  p55Boot(0);
}
...
function p55HideRegRow() {
  const regEl = document.getElementById("vehicle-reg");
  if (!regEl) return;
  const row = regEl.closest("p, li, div");
  (row || regEl).classList.add("p55-legacy-row");
  (row || regEl).setAttribute("aria-hidden", "true");
}
...
function renderVehicle(rec) {
  ...
  const resolvePhone = () =>
    String(window.P55?.whatsapp?.phoneE164 || window.WHATSAPP_NUMBER || "")
      .replace(/\D/g, "");

  let phone = resolvePhone();

  const setWhatsAppHref = () => {
    if (!waBtn) return;
    phone = resolvePhone();
    if (!phone) {
      // Hide FAB if we don't have a configured number yet
  ...
  // If the WhatsApp config script loads after this file, refresh the href once
  if (waBtn && !phone) setTimeout(setWhatsAppHref, 800);

  // Premium: icons grid + remove duplicate inline WhatsApp rows + reveal behaviour
  // (Keep DOM order stable; CSS handles responsiveness)
  p55ApplyPremiumSpecs(f);
  p55ApplyPremiumSpecsFromDom();
  p55PruneInlineWhatsAppUI();
  p55SetupFabReveal();

  // Mark as rendered (used by boot/retry logic)
  try { window.__p55VehicleRendered = true; } catch (e) {}
}
