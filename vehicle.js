/* ---------------------------------------------------------
   ...
   Version: v8.2 HOTFIX (20260109c)
--------------------------------------------------------- */

(() => {
  "use strict";

  const P55_VEHICLE_UI_VERSION = "v8.0-2026-01-04";
  ...
  function setupGallery(photos, makeModel) {
    ...
    function scrollThumbs(dir) {
      if (!thumbs) return;
      const amount = Math.max(220, Math.floor(thumbs.clientWidth * 0.65));
  ...
  /* ---------- Walkaround video (YouTube lazy embed) ---------- */

  function normaliseVideoUrl(raw) {
    if (!raw) return "";
    try {
      // Accept plain IDs too (11 chars typical) but we prioritise URL parsing
      const s = String(raw).trim();
      if (!s) return "";
      if (/^[a-zA-Z0-9_-]{8,20}$/.test(s) && !s.includes("http")) return `https://youtu.be/${s}`;
  ...
  function extractYouTubeId(url) {
    ...
    // youtu.be/<id>
    let m = s.match(/youtu\.be\/([a-zA-Z0-9_-]{8,20})/);
    if (m && m[1]) return m[1];

    // youtube.com/watch?v=<id>
    m = s.match(/[?&]v=([a-zA-Z0-9_-]{8,20})/);
    if (m && m[1]) return m[1];

    // youtube.com/shorts/<id>
    m = s.match(/youtube\.com\/shorts\/([a-zA-Z0-9_-]{8,20})/);
    if (m && m[1]) return m[1];

    // youtube.com/embed/<id>
    m = s.match(/youtube\.com\/embed\/([a-zA-Z0-9_-]{8,20})/);
    if (m && m[1]) return m[1];
  ...
  function openWalkaroundModal(youtubeId, watchUrl) {
    const modal = document.getElementById("p55-video-modal");
    const frame = document.getElementById("p55-video-modal-frame");
    const closeBtn = document.getElementById("p55-video-modal-close");
    const backdrop = document.getElementById("p55-video-modal-backdrop");
    const yt = document.getElementById("p55-video-modal-yt");
    if (!modal || !frame) return;

    // lock background scroll (premium modal behaviour)
    const prevOverflow = document.documentElement.style.overflow;
    document.documentElement.style.overflow = "hidden";

    frame.innerHTML = "";
    const iframe = document.createElement("iframe");
    iframe.src = `https://www.youtube-nocookie.com/embed/${youtubeId}?autoplay=1&mute=1&playsinline=1&rel=0&modestbranding=1`;
    iframe.title = "Walkaround video";
    iframe.allow = "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share";
    iframe.allowFullscreen = true;
    frame.appendChild(iframe);

    if (yt) yt.href = watchUrl;
  ...
  function setupWalkaroundVideo(fields, statusText) {
    const moduleEl = document.getElementById("vehicle-video");
    if (!moduleEl) return;

    const st = String(statusText || fields?.Status || "").trim() || "Available";
    const isAllowed = (st === "Available" || st === "Sold");

    const raw =
      fields?.Video_URL ||
      fields?.Walkaround_Video ||
      fields?.WalkaroundVideo ||
      fields?.Walkaround ||
      fields?.YouTube ||
      fields?.YouTube_Link ||
      fields?.YouTubeLink ||
    ...
    const open = document.getElementById("vehicle-video-open");
    const posterBtn = document.getElementById("vehicle-video-pill");
    const thumbImg = document.getElementById("vehicle-video-thumb");
const watchUrl = `https://www.youtube.com/watch?v=${encodeURIComponent(id)}`;

    if (open) open.href = watchUrl;

    // Try maxresdefault first; if missing, swap to hqdefault on error.
    if (thumbImg) {
      const maxRes = `https://i.ytimg.com/vi/${id}/maxresdefault.jpg`;
      const hq = `https://i.ytimg.com/vi/${id}/hqdefault.jpg`;
      thumbImg.src = maxRes;
      thumbImg.onerror = () => { thumbImg.onerror = null; thumbImg.src = hq; };
    }

    // Open modal only when tapped (keeps page clean + premium)
    if (posterBtn) {
      posterBtn.onclick = (e) => {
        e.preventDefault();
        openWalkaroundModal(id, watchUrl);
        try { window.P55?.sendEvent?.("walkaround_play", { id: fields?.id || "", youtubeId: id }); } catch (_) {}
      };
    }
  ...
  /* ---------- Buying confidence (trust block) ---------- */

  function setupBuyingConfidence(statusText) {
    const el = document.getElementById("vehicle-confidence");
    if (!el) return;

    const st = String(statusText || "").trim() || "Available";
    const show = (st === "Available" || st === "Arriving Soon" || st === "In Prep");
    el.hidden = !show;
  }
...
/* ---------- Main render ---------- */
  ...
  function renderVehicle(rec) {
    ...
    // Trust block
    try { setupBuyingConfidence(st); } catch (_) {}
    ...
    // Walkaround video (YouTube)
    try { setupWalkaroundVideo(f, st); } catch (_) {}
  ...
  let id = params.get("id") || params.get("record") || params.get("rec") || "";

  // Fallback if query string is lost / stripped
  if (!id) {
    try {
      id = sessionStorage.getItem("p55:lastVehicleId") || "";
    } catch (_) {}
  }
