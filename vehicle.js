...
(() => {
  ...
  function setupWalkaroundVideo(fields, statusText) {
    ...
    const frameWrap = document.getElementById("p55-video-modal-frame");
    ...
    // Lazy embed only when tapped (keeps performance + premium feel)
    if (posterBtn && frameWrap) {
      const mountIframe = () => {
        try {
      ...
      posterBtn.onclick = (e) => { e.preventDefault(); mountIframe(); };
    }
