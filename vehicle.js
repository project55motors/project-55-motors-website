...
// P55 premium vehicle UI patch version (for cache/browsers)
const P55_VEHICLE_UI_VERSION = \"v5-2026-01-01\";
...
function p55PruneLegacySpecs() {
  // Hide the original "Mileage / Transmission / MOT / Engine / Fuel" rows once we render the icon pills.
  // We MARK rows rather than removing them to avoid browser-specific DOM timing differences.
  const entries = [
  ...
  const safeDivRow = (row, label) => {
    if (!row || row.tagName !== "DIV") return false;
    const txt = (row.textContent || "").trim().toLowerCase();
    if (!txt) return false;
    // Avoid hiding large containers: keep it tight to the label/value line
    if (txt.length > 90) return false;
    if (!txt.startsWith(label)) return false;
    // Avoid removing CTA containers
  ...
  entries.forEach(({ id, label }) => {
    ...
    // Prefer semantic rows
    let row = el.closest("p, li");
    if (row) return markRow(row);

    // Common pattern: <div>Label <span id=...>Value</span></div>
    row = el.closest("div");
    if (safeDivRow(row, label)) return markRow(row);

    // Fallback: hide the direct parent if it looks like a single line
    const parent = el.parentElement;
    if (parent) {
      const ptxt = (parent.textContent || "").trim().toLowerCase();
      if (ptxt.length <= 90 && ptxt.includes(label)) markRow(parent);
    }
  });
}
...
function p55ApplyPremiumSpecsFromDom() {
  // Build icon pills from existing DOM values (template-robust)
  const regEl = document.getElementById("vehicle-reg");
  if (!regEl) return;

  // Find a reasonable container for the header area
  const container =
  ...
  // Ensure title/price row exists (keeps formatting consistent)
  const title = document.getElementById("vehicle-title");
  const price = document.getElementById("vehicle-price");
  if (title && price) {
  ...
  // Hide legacy lines now (by marking rows)
  try { p55PruneLegacySpecs(); } catch(e) {}

  // Create / update specs grid
...
function p55PruneInlineWhatsAppUI() {
  // Remove any inline WhatsApp link blocks, keeping only the floating #whatsapp-btn and the main blue Enquire CTA.
  const keepFab = document.getElementById("whatsapp-btn");

  // Remove inline WhatsApp anchors that are not the floating FAB
  document.querySelectorAll('a[href*="wa.me"], a[href*="whatsapp.com"], a[href^="whatsapp:"]').forEach(a => {
    if (keepFab && a === keepFab) return;

    const txt = (a.textContent || "").trim().toLowerCase();
    const isMainEnquire = txt.includes("enquire about this vehicle");
    if (isMainEnquire) return;

    // Remove the whole line if it looks like the legacy helper row
    const row = a.closest("p, div, section");
    if (row) row.classList.add("p55-legacy-row");
    else a.classList.add("p55-legacy-row");
  });

  // Remove legacy helper text even if it isn't a link (Edge/Firefox can render differently)
  document.querySelectorAll("p, div, span, small").forEach(el => {
    const t = (el.textContent || "").trim().toLowerCase();
    if (!t) return;
    if (t.includes("message about this car") || t.includes("quick question") || t.includes("viewing times") || t.includes("part-exchange")) {
      // Do not hide the main enquiry button container
      if (el.querySelector("a, button") && (el.textContent || "").toLowerCase().includes("enquire about this vehicle")) return;
