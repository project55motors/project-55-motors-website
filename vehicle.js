...
function p55PruneLegacySpecs() {
  // Remove the 5 legacy rows by targeting the value span IDs.
  const mileage = document.getElementById("vehicle-mileage");
  const trans = document.getElementById("vehicle-transmission");
  const mot = document.getElementById("vehicle-mot");
  const eng = document.getElementById("vehicle-engine");
  const fuel = document.getElementById("vehicle-fuel");

  const rowSelectors = "p, li, .row, .detail-row, .vehicle-row, .kv-row";

  [mileage, trans, mot, eng, fuel].forEach(el => p55RemoveClosest(el, rowSelectors));

  // As a fallback, remove <p> rows in the header that contain the same IDs.
  const header = p55FindHeaderRoot();
  if (!header) return;
  header.querySelectorAll("p").forEach(p => {
    const hasKnown = p.querySelector("#vehicle-mileage, #vehicle-transmission, #vehicle-mot, #vehicle-engine, #vehicle-fuel");
    if (hasKnown) p.remove();
  });
}
