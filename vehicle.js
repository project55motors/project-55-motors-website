/* ---------------------------------------------------------
   ...
   - Render a privacy-conscious, click-to-play YouTube walk-around
   - Keep only ONE WhatsApp CTA: the floating button (no inline remnants)
   - Enquire button pre-fills message for this vehicle

   Version: v9.0 WALK-AROUND VIDEO (20260812a)
--------------------------------------------------------- */

(() => {
  "use strict";

  const P55_VEHICLE_UI_VERSION = "v9.0-2026-08-12";
  ...
  const SELECTORS = {
    ...
    video: "#vehicle-video",
    videoPlay: "#vehicle-video-play",
    videoPoster: "#vehicle-video-poster",
  ...
  function parseYouTubeUrl(value) {
    ...
    if (/^[A-Za-z0-9_-]{11}$/.test(raw)) {
      return {
        ...
        posterUrl: `https://i.ytimg.com/vi/${raw}/maxresdefault.jpg`,
        posterFallbackUrl: `https://i.ytimg.com/vi/${raw}/hqdefault.jpg`
      };
    }
  ...
  function ensureVideoSection() {
    let section = qs(SELECTORS.video);
    if (section) return section;

    const thumbs = qs(".vehicle-thumbs-wrapper");
    if (!thumbs) return null;

    thumbs.insertAdjacentHTML("afterend", `
      <section id="vehicle-video" class="vehicle-video" aria-labelledby="vehicle-video-title" hidden>
        <div class="vehicle-video__heading">
          <div>
            <p class="vehicle-video__eyebrow">Project 55 Motors</p>
            <h2 id="vehicle-video-title">Vehicle walk-around</h2>
            <p class="vehicle-video__intro">A closer look at the condition, specification and features.</p>
          </div>
          <a id="vehicle-video-link" class="vehicle-video__link" href="#" target="_blank" rel="noopener noreferrer">Watch on YouTube <span aria-hidden="true">↗</span></a>
        </div>
        <div class="vehicle-video__frame">
          <button id="vehicle-video-play" class="vehicle-video__play" type="button">
            <img id="vehicle-video-poster" class="vehicle-video__poster" src="" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">
            <span class="vehicle-video__shade" aria-hidden="true"></span>
            <span class="vehicle-video__play-ui"><span class="vehicle-video__play-icon" aria-hidden="true"><span></span></span><span class="vehicle-video__play-label">Play walk-around</span></span>
          </button>
          <div id="vehicle-video-embed" class="vehicle-video__embed" hidden></div>
        </div>
      </section>
    `);

    return qs(SELECTORS.video);
  }

  function renderVehicleVideo(value, rec) {
    const section = ensureVideoSection();
    if (!section) return;
    ...
    const play = qs(SELECTORS.videoPlay, section);
    const poster = qs(SELECTORS.videoPoster, section);
    const embed = qs(SELECTORS.videoEmbed, section);
    const directLink = qs(SELECTORS.videoLink, section);
    const vehicleName = String(rec?.fields?.Make_Model || "this vehicle").trim() || "this vehicle";

    if (!play || !poster || !embed || !directLink) {
      section.hidden = true;
      return;
    }

    poster.dataset.fallbackUsed = "0";
    poster.src = video.posterUrl;
    poster.addEventListener("error", () => {
      if (poster.dataset.fallbackUsed === "1") return;
      poster.dataset.fallbackUsed = "1";
      poster.src = video.posterFallbackUrl;
    }, { once: true });

    play.setAttribute("aria-label", `Play walk-around video for ${vehicleName}`);
