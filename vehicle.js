/* ---------------------------------------------------------
   ...
   Version: v8.1 (2026-01-09)
--------------------------------------------------------- */

(() => {
  "use strict";

  const P55_VEHICLE_UI_VERSION = "v8.1-2026-01-09";
  ...
  const SELECTORS = {
    ...
    enquire: "#enquire-btn",
    whatsapp: "#whatsapp-btn, #whatsapp-fab, #p55-whatsapp-fab",   // support older + site.js FAB
  ...
  function setupGallery(photos, makeModel) {
    ...
    // iPad/Safari: prefer pointerdown/touchstart for small arrow buttons; guard against double-fire
    let __p55ArrowTapTs = 0;
    function bindArrowTap(btn, fn) {
      if (!btn) return;
      const fire = (e) => {
        const now = Date.now();
        if (now - __p55ArrowTapTs < 450) return;
        __p55ArrowTapTs = now;
    ...
    bindArrowTap(btnPrev, () => scrollThumbs(-1));
    bindArrowTap(btnNext, () => scrollThumbs(1));
  ...
  function renderVehicle(rec) {
    ...
    if (enquireBtn) {
      ...
      enquireBtn.addEventListener("click", () => {
        ...
        window.location.href = `/contact?msg=${encodeURIComponent(msg)}&vehicle=${encodeURIComponent(f.Make_Model || "vehicle")}&reg=${encodeURIComponent(f.Registration || "")}&url=${encodeURIComponent(window.location.href)}`;
      });
    }
    ...
    // WhatsApp floating CTA (site.js creates the button; this updates it with vehicle context)
    const phone = getWhatsAppPhoneE164();
    if (phone) {
      const msg = isSold
        ? `Hi Project 55 Motors — I saw the ${f.Make_Model || "vehicle"} (${f.Registration || "registration unknown"}) you have just sold. Could you source something similar?\n\nLink: ${window.location.href}`
      ...
      // Update global default message so site.js uses the best context on this page
      window.P55 = window.P55 || {};
      window.P55.whatsapp = window.P55.whatsapp || {};
      window.P55.whatsapp.defaultMessage = msg;

      // Update any existing WhatsApp anchor (site.js: #p55-whatsapp-fab; legacy templates: #whatsapp-btn/#whatsapp-fab)
      const waEls = Array.from(document.querySelectorAll(SELECTORS.whatsapp));
      waEls.forEach((el) => {
        try {
          el.setAttribute("href", `https://wa.me/${phone}?text=${encodeURIComponent(msg)}`);
          el.setAttribute("aria-label", "Message Project 55 Motors on WhatsApp");
          el.setAttribute("title", "Message on WhatsApp");
        } catch (_) {}
      });
    }
    }
  ...
  async function loadVehicle() {
    ...
    // Canonical URL (prevents duplicate indexing across www/non-www and supports shareable vehicle URLs)
    try {
      const base = "https://project55motors.co.uk";
      const canonicalHref = `${base}/vehicle.html?id=${encodeURIComponent(id)}`;
