...
const P55_VEHICLE_UI_VERSION = "v6.8-2026-01-01";
...
function p55GetFabEl() {
  // Prefer the dedicated floating FAB element. Avoid grabbing legacy inline WhatsApp links.
  const explicit = document.querySelector(".whatsapp-fab") || document.getElementById("whatsapp-fab");
  if (explicit) return explicit;

  const legacy = document.getElementById("whatsapp-btn");
  if (!legacy) return null;

  const ok = legacy.classList.contains("whatsapp-fab") || legacy.classList.contains("whatsapp-btn") || legacy.dataset.fab === "1";
